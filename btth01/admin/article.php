...
<body>
    ...
    if (isset($_GET['delete'])) {
        ...
        if ($conn->query($sql) === TRUE) {
            ...
            echo "<script>alert('Xóa không thành công!');</script>" . $conn->error;
        }
    }

    $sql = "SELECT baiviet.ma_bviet, baiviet.tieude, baiviet.ten_bhat, baiviet.tomtat, baiviet.noidung, theloai.ten_tloai, tacgia.ten_tgia, baiviet.ngayviet, baiviet.hinhanh
            FROM baiviet
                    ...
                    <tbody>
                        ...
                        if ($result->num_rows > 0) {
                            while($row = $result->fetch_assoc()) {
                                ...
                                //echo "<td><img src='uploads/{$row['hinhanh']}' alt='Image' width='100'></td>";
                                if (!empty($row['hinhanh'])) {
                                    echo "<td><img src='uploads/{$row['hinhanh']}' alt='Image' width='100'></td>";
                                } else {
                                    echo "<td>". $row["hinhanh"] . "</td>";  // Hoặc để trống nếu không muốn hiển thị gì
                                }
                                echo '<td><a href="edit_article.php?id=' . $row['ma_bviet'] . '"><i class="fa-solid fa-pen-to-square"></i></a></td>';
                                echo '<td><a href="article.php?delete='. $row['ma_bviet'] . '"><i class="fa-solid fa-trash"></i></a></td>';
                                echo "</tr>";
                            }
                        } else {
